...
def index(request):
    ...
    context = {
        'most_popular_posts':
            [serialize_popular_post(post) for post in most_popular_posts],
        'page_posts':
            [serialize_fresh_post(post) for post in most_fresh_posts],
        'popular_tags':
...
def post_detail(request, slug):
    ...
    serialized_post = {
        ...
        'published_at': post.published_at,
        'slug': post.slug,
        'tags': [serialize_tag_in_post(tag) for tag in related_tags],
    }
...
def tag_filter(request, tag_title):
    ...
    most_related_posts = related_posts.fresh(20).fetch_with_comments_count()\
        .prefetch_related('tags')
    ...
    context = {
        ...
        'popular_tags':
            [serialize_tag(tag) for tag in most_popular_tags],
        'posts':
            [serialize_fresh_post(post) for post in most_related_posts],
        'most_popular_posts':
            [serialize_popular_post(post) for post in most_popular_posts],
    }
